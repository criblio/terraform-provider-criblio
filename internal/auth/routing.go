...
var cloudOnlyPaths = map[string]bool{}
var onPremOnlyPaths = map[string]bool{}

func IsRestrictedOnPremEndpoint(path string) bool {
	return pathInAvailabilitySet(path, cloudOnlyPaths)
}

// IsRestrictedCloudEndpoint reports whether path is unavailable in Cloud.
// The onPremOnlyPaths map is generated from x-cribl-availability annotations.
func IsRestrictedCloudEndpoint(path string) bool {
	return pathInAvailabilitySet(path, onPremOnlyPaths)
}

func pathInAvailabilitySet(path string, paths map[string]bool) bool {
