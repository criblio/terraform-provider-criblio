// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetLicenseByIDServerCloud        string = "cloud"
	GetLicenseByIDServerCloudGroup   string = "cloud-group"
	GetLicenseByIDServerManagedGroup string = "managed-group"
)

var GetLicenseByIDServerList = map[string]string{
	GetLicenseByIDServerCloud:        "https://app.cribl.cloud",
	GetLicenseByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetLicenseByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetLicenseByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetLicenseByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetLicenseByIDResponseBody - a list of License objects
type GetLicenseByIDResponseBody struct {
	Items []shared.License `json:"items,omitempty"`
}

func (o *GetLicenseByIDResponseBody) GetItems() []shared.License {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetLicenseByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of License objects
	Object *GetLicenseByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetLicenseByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetLicenseByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetLicenseByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetLicenseByIDResponse) GetObject() *GetLicenseByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetLicenseByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
