// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetSystemProjectsVersionShowByProjectIDServerCloud        string = "cloud"
	GetSystemProjectsVersionShowByProjectIDServerCloudGroup   string = "cloud-group"
	GetSystemProjectsVersionShowByProjectIDServerManagedGroup string = "managed-group"
)

var GetSystemProjectsVersionShowByProjectIDServerList = map[string]string{
	GetSystemProjectsVersionShowByProjectIDServerCloud:        "https://app.cribl.cloud",
	GetSystemProjectsVersionShowByProjectIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetSystemProjectsVersionShowByProjectIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetSystemProjectsVersionShowByProjectIDRequest struct {
	// Project Id
	ProjectID string `pathParam:"style=simple,explode=false,name=projectId"`
	// Commit ID
	ID *string `queryParam:"style=form,explode=true,name=ID"`
	// Filename
	Filename *string `queryParam:"style=form,explode=true,name=filename"`
	// Limit maximum lines in the diff
	DiffLineLimit *float64 `queryParam:"style=form,explode=true,name=diffLineLimit"`
}

func (o *GetSystemProjectsVersionShowByProjectIDRequest) GetProjectID() string {
	if o == nil {
		return ""
	}
	return o.ProjectID
}

func (o *GetSystemProjectsVersionShowByProjectIDRequest) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

func (o *GetSystemProjectsVersionShowByProjectIDRequest) GetFilename() *string {
	if o == nil {
		return nil
	}
	return o.Filename
}

func (o *GetSystemProjectsVersionShowByProjectIDRequest) GetDiffLineLimit() *float64 {
	if o == nil {
		return nil
	}
	return o.DiffLineLimit
}

// GetSystemProjectsVersionShowByProjectIDResponseBody - a list of any objects
type GetSystemProjectsVersionShowByProjectIDResponseBody struct {
	Items []map[string]any `json:"items,omitempty"`
}

func (o *GetSystemProjectsVersionShowByProjectIDResponseBody) GetItems() []map[string]any {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetSystemProjectsVersionShowByProjectIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of any objects
	Object *GetSystemProjectsVersionShowByProjectIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetSystemProjectsVersionShowByProjectIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetSystemProjectsVersionShowByProjectIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetSystemProjectsVersionShowByProjectIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetSystemProjectsVersionShowByProjectIDResponse) GetObject() *GetSystemProjectsVersionShowByProjectIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetSystemProjectsVersionShowByProjectIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
