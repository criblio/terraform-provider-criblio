// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateCertificateByIDServerCloud        string = "cloud"
	UpdateCertificateByIDServerCloudGroup   string = "cloud-group"
	UpdateCertificateByIDServerManagedGroup string = "managed-group"
)

var UpdateCertificateByIDServerList = map[string]string{
	UpdateCertificateByIDServerCloud:        "https://app.cribl.cloud",
	UpdateCertificateByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateCertificateByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateCertificateByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Certificate object to be updated
	Certificate shared.Certificate `request:"mediaType=application/json"`
}

func (o *UpdateCertificateByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateCertificateByIDRequest) GetCertificate() shared.Certificate {
	if o == nil {
		return shared.Certificate{}
	}
	return o.Certificate
}

// UpdateCertificateByIDResponseBody - a list of Certificate objects
type UpdateCertificateByIDResponseBody struct {
	Items []shared.Certificate `json:"items,omitempty"`
}

func (o *UpdateCertificateByIDResponseBody) GetItems() []shared.Certificate {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateCertificateByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Certificate objects
	Object *UpdateCertificateByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateCertificateByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateCertificateByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateCertificateByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateCertificateByIDResponse) GetObject() *UpdateCertificateByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateCertificateByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
