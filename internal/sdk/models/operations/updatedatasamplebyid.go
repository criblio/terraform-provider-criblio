// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateDataSampleByIDServerCloud        string = "cloud"
	UpdateDataSampleByIDServerCloudGroup   string = "cloud-group"
	UpdateDataSampleByIDServerManagedGroup string = "managed-group"
)

var UpdateDataSampleByIDServerList = map[string]string{
	UpdateDataSampleByIDServerCloud:        "https://app.cribl.cloud",
	UpdateDataSampleByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateDataSampleByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateDataSampleByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// DataSample object to be updated
	DataSample shared.DataSample `request:"mediaType=application/json"`
}

func (o *UpdateDataSampleByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateDataSampleByIDRequest) GetDataSample() shared.DataSample {
	if o == nil {
		return shared.DataSample{}
	}
	return o.DataSample
}

// UpdateDataSampleByIDResponseBody - a list of DataSample objects
type UpdateDataSampleByIDResponseBody struct {
	Items []shared.DataSample `json:"items,omitempty"`
}

func (o *UpdateDataSampleByIDResponseBody) GetItems() []shared.DataSample {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateDataSampleByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of DataSample objects
	Object *UpdateDataSampleByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateDataSampleByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateDataSampleByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateDataSampleByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateDataSampleByIDResponse) GetObject() *UpdateDataSampleByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateDataSampleByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
