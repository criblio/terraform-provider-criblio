// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateOutputByIDServerCloud        string = "cloud"
	UpdateOutputByIDServerCloudGroup   string = "cloud-group"
	UpdateOutputByIDServerManagedGroup string = "managed-group"
)

var UpdateOutputByIDServerList = map[string]string{
	UpdateOutputByIDServerCloud:        "https://app.cribl.cloud",
	UpdateOutputByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateOutputByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateOutputByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
	// Output object to be updated
	Output shared.Output `request:"mediaType=application/json"`
}

func (o *UpdateOutputByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateOutputByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

func (o *UpdateOutputByIDRequest) GetOutput() shared.Output {
	if o == nil {
		return shared.Output{}
	}
	return o.Output
}

// UpdateOutputByIDResponseBody - a list of Output objects
type UpdateOutputByIDResponseBody struct {
}

type UpdateOutputByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Output objects
	Object *UpdateOutputByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateOutputByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateOutputByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateOutputByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateOutputByIDResponse) GetObject() *UpdateOutputByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateOutputByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
