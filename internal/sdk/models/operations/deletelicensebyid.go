// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteLicenseByIDServerCloud        string = "cloud"
	DeleteLicenseByIDServerCloudGroup   string = "cloud-group"
	DeleteLicenseByIDServerManagedGroup string = "managed-group"
)

var DeleteLicenseByIDServerList = map[string]string{
	DeleteLicenseByIDServerCloud:        "https://app.cribl.cloud",
	DeleteLicenseByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteLicenseByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteLicenseByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteLicenseByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteLicenseByIDResponseBody - a list of License objects
type DeleteLicenseByIDResponseBody struct {
	Items []shared.License `json:"items,omitempty"`
}

func (o *DeleteLicenseByIDResponseBody) GetItems() []shared.License {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteLicenseByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of License objects
	Object *DeleteLicenseByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteLicenseByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteLicenseByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteLicenseByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteLicenseByIDResponse) GetObject() *DeleteLicenseByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteLicenseByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
