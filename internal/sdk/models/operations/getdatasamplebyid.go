// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetDataSampleByIDServerCloud        string = "cloud"
	GetDataSampleByIDServerCloudGroup   string = "cloud-group"
	GetDataSampleByIDServerManagedGroup string = "managed-group"
)

var GetDataSampleByIDServerList = map[string]string{
	GetDataSampleByIDServerCloud:        "https://app.cribl.cloud",
	GetDataSampleByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetDataSampleByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetDataSampleByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetDataSampleByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetDataSampleByIDResponseBody - a list of DataSample objects
type GetDataSampleByIDResponseBody struct {
	Items []shared.DataSample `json:"items,omitempty"`
}

func (o *GetDataSampleByIDResponseBody) GetItems() []shared.DataSample {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetDataSampleByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of DataSample objects
	Object *GetDataSampleByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetDataSampleByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetDataSampleByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetDataSampleByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetDataSampleByIDResponse) GetObject() *GetDataSampleByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetDataSampleByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
