// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteUsageGroupByIDServerCloud        string = "cloud"
	DeleteUsageGroupByIDServerCloudGroup   string = "cloud-group"
	DeleteUsageGroupByIDServerManagedGroup string = "managed-group"
)

var DeleteUsageGroupByIDServerList = map[string]string{
	DeleteUsageGroupByIDServerCloud:        "https://app.cribl.cloud",
	DeleteUsageGroupByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteUsageGroupByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteUsageGroupByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteUsageGroupByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteUsageGroupByIDResponseBody - a list of UsageGroup objects
type DeleteUsageGroupByIDResponseBody struct {
	Items []shared.UsageGroup `json:"items,omitempty"`
}

func (o *DeleteUsageGroupByIDResponseBody) GetItems() []shared.UsageGroup {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteUsageGroupByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of UsageGroup objects
	Object *DeleteUsageGroupByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteUsageGroupByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteUsageGroupByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteUsageGroupByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteUsageGroupByIDResponse) GetObject() *DeleteUsageGroupByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteUsageGroupByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
