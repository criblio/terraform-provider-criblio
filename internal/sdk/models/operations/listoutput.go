// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	ListOutputServerCloud        string = "cloud"
	ListOutputServerCloudGroup   string = "cloud-group"
	ListOutputServerManagedGroup string = "managed-group"
)

var ListOutputServerList = map[string]string{
	ListOutputServerCloud:        "https://app.cribl.cloud",
	ListOutputServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	ListOutputServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type ListOutputRequest struct {
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *ListOutputRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// ListOutputResponseBody - a list of Output objects
type ListOutputResponseBody struct {
}

type ListOutputResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Output objects
	Object *ListOutputResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *ListOutputResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *ListOutputResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *ListOutputResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *ListOutputResponse) GetObject() *ListOutputResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *ListOutputResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
