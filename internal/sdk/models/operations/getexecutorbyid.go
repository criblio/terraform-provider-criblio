// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetExecutorByIDServerCloud        string = "cloud"
	GetExecutorByIDServerCloudGroup   string = "cloud-group"
	GetExecutorByIDServerManagedGroup string = "managed-group"
)

var GetExecutorByIDServerList = map[string]string{
	GetExecutorByIDServerCloud:        "https://app.cribl.cloud",
	GetExecutorByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetExecutorByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetExecutorByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetExecutorByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetExecutorByIDResponseBody - a list of Executor objects
type GetExecutorByIDResponseBody struct {
	Items []shared.Executor `json:"items,omitempty"`
}

func (o *GetExecutorByIDResponseBody) GetItems() []shared.Executor {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetExecutorByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Executor objects
	Object *GetExecutorByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetExecutorByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetExecutorByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetExecutorByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetExecutorByIDResponse) GetObject() *GetExecutorByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetExecutorByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
