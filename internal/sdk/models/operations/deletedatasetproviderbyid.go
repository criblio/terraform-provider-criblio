// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteDatasetProviderByIDServerCloud        string = "cloud"
	DeleteDatasetProviderByIDServerCloudGroup   string = "cloud-group"
	DeleteDatasetProviderByIDServerManagedGroup string = "managed-group"
)

var DeleteDatasetProviderByIDServerList = map[string]string{
	DeleteDatasetProviderByIDServerCloud:        "https://app.cribl.cloud",
	DeleteDatasetProviderByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteDatasetProviderByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteDatasetProviderByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteDatasetProviderByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteDatasetProviderByIDResponseBody - a list of DatasetProvider objects
type DeleteDatasetProviderByIDResponseBody struct {
	Items []shared.GenericProvider `json:"items,omitempty"`
}

func (o *DeleteDatasetProviderByIDResponseBody) GetItems() []shared.GenericProvider {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteDatasetProviderByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of DatasetProvider objects
	Object *DeleteDatasetProviderByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteDatasetProviderByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteDatasetProviderByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteDatasetProviderByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteDatasetProviderByIDResponse) GetObject() *DeleteDatasetProviderByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteDatasetProviderByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
