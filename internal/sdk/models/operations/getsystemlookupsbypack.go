// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetSystemLookupsByPackServerCloud        string = "cloud"
	GetSystemLookupsByPackServerCloudGroup   string = "cloud-group"
	GetSystemLookupsByPackServerManagedGroup string = "managed-group"
)

var GetSystemLookupsByPackServerList = map[string]string{
	GetSystemLookupsByPackServerCloud:        "https://app.cribl.cloud",
	GetSystemLookupsByPackServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetSystemLookupsByPackServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetSystemLookupsByPackRequest struct {
	// pack ID to GET
	Pack string `pathParam:"style=simple,explode=false,name=pack"`
	// group Id
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *GetSystemLookupsByPackRequest) GetPack() string {
	if o == nil {
		return ""
	}
	return o.Pack
}

func (o *GetSystemLookupsByPackRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// GetSystemLookupsByPackResponseBody - a list of Routes objects
type GetSystemLookupsByPackResponseBody struct {
	Items []shared.Routes `json:"items,omitempty"`
}

func (o *GetSystemLookupsByPackResponseBody) GetItems() []shared.Routes {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetSystemLookupsByPackResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Routes objects
	Object *GetSystemLookupsByPackResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetSystemLookupsByPackResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetSystemLookupsByPackResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetSystemLookupsByPackResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetSystemLookupsByPackResponse) GetObject() *GetSystemLookupsByPackResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetSystemLookupsByPackResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
