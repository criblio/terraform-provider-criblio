// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetTeamByIDServerCloud        string = "cloud"
	GetTeamByIDServerCloudGroup   string = "cloud-group"
	GetTeamByIDServerManagedGroup string = "managed-group"
)

var GetTeamByIDServerList = map[string]string{
	GetTeamByIDServerCloud:        "https://app.cribl.cloud",
	GetTeamByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetTeamByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetTeamByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetTeamByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetTeamByIDResponseBody - a list of Team objects
type GetTeamByIDResponseBody struct {
	Items []shared.Team `json:"items,omitempty"`
}

func (o *GetTeamByIDResponseBody) GetItems() []shared.Team {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetTeamByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Team objects
	Object *GetTeamByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetTeamByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetTeamByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetTeamByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetTeamByIDResponse) GetObject() *GetTeamByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetTeamByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
