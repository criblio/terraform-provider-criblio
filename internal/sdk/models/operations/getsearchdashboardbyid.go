// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetSearchDashboardByIDServerCloud        string = "cloud"
	GetSearchDashboardByIDServerCloudGroup   string = "cloud-group"
	GetSearchDashboardByIDServerManagedGroup string = "managed-group"
)

var GetSearchDashboardByIDServerList = map[string]string{
	GetSearchDashboardByIDServerCloud:        "https://app.cribl.cloud",
	GetSearchDashboardByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetSearchDashboardByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetSearchDashboardByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetSearchDashboardByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetSearchDashboardByIDResponseBody - a list of SearchDashboard objects
type GetSearchDashboardByIDResponseBody struct {
	Items []shared.SearchDashboard `json:"items,omitempty"`
}

func (o *GetSearchDashboardByIDResponseBody) GetItems() []shared.SearchDashboard {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetSearchDashboardByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of SearchDashboard objects
	Object *GetSearchDashboardByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetSearchDashboardByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetSearchDashboardByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetSearchDashboardByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetSearchDashboardByIDResponse) GetObject() *GetSearchDashboardByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetSearchDashboardByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
