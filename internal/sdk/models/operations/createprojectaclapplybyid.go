// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	CreateProjectACLApplyByIDServerCloud        string = "cloud"
	CreateProjectACLApplyByIDServerCloudGroup   string = "cloud-group"
	CreateProjectACLApplyByIDServerManagedGroup string = "managed-group"
)

var CreateProjectACLApplyByIDServerList = map[string]string{
	CreateProjectACLApplyByIDServerCloud:        "https://app.cribl.cloud",
	CreateProjectACLApplyByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	CreateProjectACLApplyByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type CreateProjectACLApplyByIDRequest struct {
	// Unique ID for ACL Create
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// AccessControlSchema object
	AccessControlSchema shared.AccessControlSchema `request:"mediaType=application/json"`
}

func (o *CreateProjectACLApplyByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *CreateProjectACLApplyByIDRequest) GetAccessControlSchema() shared.AccessControlSchema {
	if o == nil {
		return shared.AccessControlSchema{}
	}
	return o.AccessControlSchema
}

type CreateProjectACLApplyByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// Unexpected error
	Error *shared.Error
}

func (o *CreateProjectACLApplyByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *CreateProjectACLApplyByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *CreateProjectACLApplyByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *CreateProjectACLApplyByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
