// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetVersionStatusServerCloud        string = "cloud"
	GetVersionStatusServerCloudGroup   string = "cloud-group"
	GetVersionStatusServerManagedGroup string = "managed-group"
)

var GetVersionStatusServerList = map[string]string{
	GetVersionStatusServerCloud:        "https://app.cribl.cloud",
	GetVersionStatusServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetVersionStatusServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetVersionStatusRequest struct {
	// Group ID
	Group *string `queryParam:"style=form,explode=true,name=group"`
}

func (o *GetVersionStatusRequest) GetGroup() *string {
	if o == nil {
		return nil
	}
	return o.Group
}

// GetVersionStatusResponseBody - a list of GitStatusResult objects
type GetVersionStatusResponseBody struct {
	Items []shared.GitStatusResult `json:"items,omitempty"`
}

func (o *GetVersionStatusResponseBody) GetItems() []shared.GitStatusResult {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetVersionStatusResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of GitStatusResult objects
	Object *GetVersionStatusResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetVersionStatusResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetVersionStatusResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetVersionStatusResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetVersionStatusResponse) GetObject() *GetVersionStatusResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetVersionStatusResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
