// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetSystemJobsLogsByIDAndGroupIDServerCloud        string = "cloud"
	GetSystemJobsLogsByIDAndGroupIDServerCloudGroup   string = "cloud-group"
	GetSystemJobsLogsByIDAndGroupIDServerManagedGroup string = "managed-group"
)

var GetSystemJobsLogsByIDAndGroupIDServerList = map[string]string{
	GetSystemJobsLogsByIDAndGroupIDServerCloud:        "https://app.cribl.cloud",
	GetSystemJobsLogsByIDAndGroupIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetSystemJobsLogsByIDAndGroupIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetSystemJobsLogsByIDAndGroupIDRequest struct {
	// Job id
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Maximum number of log lines to retrieve starting from offset.
	Limit *int64 `queryParam:"style=form,explode=true,name=limit"`
	// Offset in the current log file to fetch the log events from.
	Offset *int64 `queryParam:"style=form,explode=true,name=offset"`
	// in the current log file to fetch the log events upto.
	EndOffset *int64 `queryParam:"style=form,explode=true,name=endOffset"`
	// Epoch timestamp of the earliest event (includes rolled files present on disk)
	Et *int64 `queryParam:"style=form,explode=true,name=et"`
	// Epoch timestamp of the latest event (includes rolled files present on disk)
	Lt *int64 `queryParam:"style=form,explode=true,name=lt"`
	// Filter
	Filter *string `queryParam:"style=form,explode=true,name=filter"`
	// Group ID (optional)
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetLimit() *int64 {
	if o == nil {
		return nil
	}
	return o.Limit
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetOffset() *int64 {
	if o == nil {
		return nil
	}
	return o.Offset
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetEndOffset() *int64 {
	if o == nil {
		return nil
	}
	return o.EndOffset
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetEt() *int64 {
	if o == nil {
		return nil
	}
	return o.Et
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetLt() *int64 {
	if o == nil {
		return nil
	}
	return o.Lt
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetFilter() *string {
	if o == nil {
		return nil
	}
	return o.Filter
}

func (o *GetSystemJobsLogsByIDAndGroupIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// GetSystemJobsLogsByIDAndGroupIDResponseBody - a list of any objects
type GetSystemJobsLogsByIDAndGroupIDResponseBody struct {
	Items []map[string]any `json:"items,omitempty"`
}

func (o *GetSystemJobsLogsByIDAndGroupIDResponseBody) GetItems() []map[string]any {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetSystemJobsLogsByIDAndGroupIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of any objects
	Object *GetSystemJobsLogsByIDAndGroupIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetSystemJobsLogsByIDAndGroupIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetSystemJobsLogsByIDAndGroupIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetSystemJobsLogsByIDAndGroupIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetSystemJobsLogsByIDAndGroupIDResponse) GetObject() *GetSystemJobsLogsByIDAndGroupIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetSystemJobsLogsByIDAndGroupIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
