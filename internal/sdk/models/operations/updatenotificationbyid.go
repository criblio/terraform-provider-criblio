// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateNotificationByIDServerCloud        string = "cloud"
	UpdateNotificationByIDServerCloudGroup   string = "cloud-group"
	UpdateNotificationByIDServerManagedGroup string = "managed-group"
)

var UpdateNotificationByIDServerList = map[string]string{
	UpdateNotificationByIDServerCloud:        "https://app.cribl.cloud",
	UpdateNotificationByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateNotificationByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateNotificationByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Notification object to be updated
	Notification shared.Notification `request:"mediaType=application/json"`
}

func (o *UpdateNotificationByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateNotificationByIDRequest) GetNotification() shared.Notification {
	if o == nil {
		return shared.Notification{}
	}
	return o.Notification
}

// UpdateNotificationByIDResponseBody - a list of Notification objects
type UpdateNotificationByIDResponseBody struct {
	Items []shared.Notification `json:"items,omitempty"`
}

func (o *UpdateNotificationByIDResponseBody) GetItems() []shared.Notification {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateNotificationByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Notification objects
	Object *UpdateNotificationByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateNotificationByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateNotificationByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateNotificationByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateNotificationByIDResponse) GetObject() *UpdateNotificationByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateNotificationByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
