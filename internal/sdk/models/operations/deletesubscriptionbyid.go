// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteSubscriptionByIDServerCloud        string = "cloud"
	DeleteSubscriptionByIDServerCloudGroup   string = "cloud-group"
	DeleteSubscriptionByIDServerManagedGroup string = "managed-group"
)

var DeleteSubscriptionByIDServerList = map[string]string{
	DeleteSubscriptionByIDServerCloud:        "https://app.cribl.cloud",
	DeleteSubscriptionByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteSubscriptionByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteSubscriptionByIDRequest struct {
	// Subscription ID
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *DeleteSubscriptionByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *DeleteSubscriptionByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// DeleteSubscriptionByIDResponseBody - a list of Subscription objects
type DeleteSubscriptionByIDResponseBody struct {
	Items []shared.Subscription `json:"items,omitempty"`
}

func (o *DeleteSubscriptionByIDResponseBody) GetItems() []shared.Subscription {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteSubscriptionByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Subscription objects
	Object *DeleteSubscriptionByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteSubscriptionByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteSubscriptionByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteSubscriptionByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteSubscriptionByIDResponse) GetObject() *DeleteSubscriptionByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteSubscriptionByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
