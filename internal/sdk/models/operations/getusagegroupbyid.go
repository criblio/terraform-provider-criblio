// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetUsageGroupByIDServerCloud        string = "cloud"
	GetUsageGroupByIDServerCloudGroup   string = "cloud-group"
	GetUsageGroupByIDServerManagedGroup string = "managed-group"
)

var GetUsageGroupByIDServerList = map[string]string{
	GetUsageGroupByIDServerCloud:        "https://app.cribl.cloud",
	GetUsageGroupByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetUsageGroupByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetUsageGroupByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetUsageGroupByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetUsageGroupByIDResponseBody - a list of UsageGroup objects
type GetUsageGroupByIDResponseBody struct {
	Items []shared.UsageGroup `json:"items,omitempty"`
}

func (o *GetUsageGroupByIDResponseBody) GetItems() []shared.UsageGroup {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetUsageGroupByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of UsageGroup objects
	Object *GetUsageGroupByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetUsageGroupByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetUsageGroupByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetUsageGroupByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetUsageGroupByIDResponse) GetObject() *GetUsageGroupByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetUsageGroupByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
