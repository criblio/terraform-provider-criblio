// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteUserByIDServerCloud        string = "cloud"
	DeleteUserByIDServerCloudGroup   string = "cloud-group"
	DeleteUserByIDServerManagedGroup string = "managed-group"
)

var DeleteUserByIDServerList = map[string]string{
	DeleteUserByIDServerCloud:        "https://app.cribl.cloud",
	DeleteUserByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteUserByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteUserByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteUserByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteUserByIDResponseBody - a list of User objects
type DeleteUserByIDResponseBody struct {
	Items []shared.User `json:"items,omitempty"`
}

func (o *DeleteUserByIDResponseBody) GetItems() []shared.User {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteUserByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of User objects
	Object *DeleteUserByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteUserByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteUserByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteUserByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteUserByIDResponse) GetObject() *DeleteUserByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteUserByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
