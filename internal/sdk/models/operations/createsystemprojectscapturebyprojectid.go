// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	CreateSystemProjectsCaptureByProjectIDServerCloud        string = "cloud"
	CreateSystemProjectsCaptureByProjectIDServerCloudGroup   string = "cloud-group"
	CreateSystemProjectsCaptureByProjectIDServerManagedGroup string = "managed-group"
)

var CreateSystemProjectsCaptureByProjectIDServerList = map[string]string{
	CreateSystemProjectsCaptureByProjectIDServerCloud:        "https://app.cribl.cloud",
	CreateSystemProjectsCaptureByProjectIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	CreateSystemProjectsCaptureByProjectIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type CreateSystemProjectsCaptureByProjectIDRequest struct {
	// Project ID
	ProjectID string `pathParam:"style=simple,explode=false,name=projectId"`
	// CaptureParams object
	CaptureParams shared.CaptureParams `request:"mediaType=application/json"`
}

func (o *CreateSystemProjectsCaptureByProjectIDRequest) GetProjectID() string {
	if o == nil {
		return ""
	}
	return o.ProjectID
}

func (o *CreateSystemProjectsCaptureByProjectIDRequest) GetCaptureParams() shared.CaptureParams {
	if o == nil {
		return shared.CaptureParams{}
	}
	return o.CaptureParams
}

// CreateSystemProjectsCaptureByProjectIDResponseBody - a list of any objects
type CreateSystemProjectsCaptureByProjectIDResponseBody struct {
	Items []map[string]any `json:"items,omitempty"`
}

func (o *CreateSystemProjectsCaptureByProjectIDResponseBody) GetItems() []map[string]any {
	if o == nil {
		return nil
	}
	return o.Items
}

type CreateSystemProjectsCaptureByProjectIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of any objects
	Object *CreateSystemProjectsCaptureByProjectIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *CreateSystemProjectsCaptureByProjectIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *CreateSystemProjectsCaptureByProjectIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *CreateSystemProjectsCaptureByProjectIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *CreateSystemProjectsCaptureByProjectIDResponse) GetObject() *CreateSystemProjectsCaptureByProjectIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *CreateSystemProjectsCaptureByProjectIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
