// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateTeamByIDServerCloud        string = "cloud"
	UpdateTeamByIDServerCloudGroup   string = "cloud-group"
	UpdateTeamByIDServerManagedGroup string = "managed-group"
)

var UpdateTeamByIDServerList = map[string]string{
	UpdateTeamByIDServerCloud:        "https://app.cribl.cloud",
	UpdateTeamByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateTeamByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateTeamByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Team object to be updated
	Team shared.Team `request:"mediaType=application/json"`
}

func (o *UpdateTeamByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateTeamByIDRequest) GetTeam() shared.Team {
	if o == nil {
		return shared.Team{}
	}
	return o.Team
}

// UpdateTeamByIDResponseBody - a list of Team objects
type UpdateTeamByIDResponseBody struct {
	Items []shared.Team `json:"items,omitempty"`
}

func (o *UpdateTeamByIDResponseBody) GetItems() []shared.Team {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateTeamByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Team objects
	Object *UpdateTeamByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateTeamByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateTeamByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateTeamByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateTeamByIDResponse) GetObject() *UpdateTeamByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateTeamByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
