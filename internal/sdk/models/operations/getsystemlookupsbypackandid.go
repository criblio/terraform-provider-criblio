// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

type GetSystemLookupsByPackAndIDRequest struct {
	// Unique ID to GET for pack
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// pack ID to GET
	Pack string `pathParam:"style=simple,explode=false,name=pack"`
	// group Id
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (g *GetSystemLookupsByPackAndIDRequest) GetID() string {
	if g == nil {
		return ""
	}
	return g.ID
}

func (g *GetSystemLookupsByPackAndIDRequest) GetPack() string {
	if g == nil {
		return ""
	}
	return g.Pack
}

func (g *GetSystemLookupsByPackAndIDRequest) GetGroupID() string {
	if g == nil {
		return ""
	}
	return g.GroupID
}

// GetSystemLookupsByPackAndIDResponseBody - LookupFile object within a Pack
type GetSystemLookupsByPackAndIDResponseBody struct {
	Items []shared.LookupFile `json:"items,omitempty"`
}

func (g *GetSystemLookupsByPackAndIDResponseBody) GetItems() []shared.LookupFile {
	if g == nil {
		return nil
	}
	return g.Items
}

type GetSystemLookupsByPackAndIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// LookupFile object within a Pack
	Object *GetSystemLookupsByPackAndIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (g *GetSystemLookupsByPackAndIDResponse) GetContentType() string {
	if g == nil {
		return ""
	}
	return g.ContentType
}

func (g *GetSystemLookupsByPackAndIDResponse) GetStatusCode() int {
	if g == nil {
		return 0
	}
	return g.StatusCode
}

func (g *GetSystemLookupsByPackAndIDResponse) GetRawResponse() *http.Response {
	if g == nil {
		return nil
	}
	return g.RawResponse
}

func (g *GetSystemLookupsByPackAndIDResponse) GetObject() *GetSystemLookupsByPackAndIDResponseBody {
	if g == nil {
		return nil
	}
	return g.Object
}

func (g *GetSystemLookupsByPackAndIDResponse) GetError() *shared.Error {
	if g == nil {
		return nil
	}
	return g.Error
}
