// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetLibSchemasByIDServerCloud        string = "cloud"
	GetLibSchemasByIDServerCloudGroup   string = "cloud-group"
	GetLibSchemasByIDServerManagedGroup string = "managed-group"
)

var GetLibSchemasByIDServerList = map[string]string{
	GetLibSchemasByIDServerCloud:        "https://app.cribl.cloud",
	GetLibSchemasByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetLibSchemasByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetLibSchemasByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *GetLibSchemasByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetLibSchemasByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// GetLibSchemasByIDResponseBody - a list of Schema objects
type GetLibSchemasByIDResponseBody struct {
	Items []shared.SchemaLibEntry `json:"items,omitempty"`
}

func (o *GetLibSchemasByIDResponseBody) GetItems() []shared.SchemaLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetLibSchemasByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Schema objects
	Object *GetLibSchemasByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetLibSchemasByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetLibSchemasByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetLibSchemasByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetLibSchemasByIDResponse) GetObject() *GetLibSchemasByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetLibSchemasByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
