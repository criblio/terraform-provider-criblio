// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetProjectByIDServerCloud        string = "cloud"
	GetProjectByIDServerCloudGroup   string = "cloud-group"
	GetProjectByIDServerManagedGroup string = "managed-group"
)

var GetProjectByIDServerList = map[string]string{
	GetProjectByIDServerCloud:        "https://app.cribl.cloud",
	GetProjectByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetProjectByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetProjectByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *GetProjectByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetProjectByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// GetProjectByIDResponseBody - a list of Project objects
type GetProjectByIDResponseBody struct {
	Items []shared.ProjectConfig `json:"items,omitempty"`
}

func (o *GetProjectByIDResponseBody) GetItems() []shared.ProjectConfig {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetProjectByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Project objects
	Object *GetProjectByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetProjectByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetProjectByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetProjectByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetProjectByIDResponse) GetObject() *GetProjectByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetProjectByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
