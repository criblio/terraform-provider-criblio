// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetPipelinesByPackWithIDServerCloud        string = "cloud"
	GetPipelinesByPackWithIDServerCloudGroup   string = "cloud-group"
	GetPipelinesByPackWithIDServerManagedGroup string = "managed-group"
)

var GetPipelinesByPackWithIDServerList = map[string]string{
	GetPipelinesByPackWithIDServerCloud:        "https://app.cribl.cloud",
	GetPipelinesByPackWithIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetPipelinesByPackWithIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetPipelinesByPackWithIDRequest struct {
	// pack ID to GET
	Pack string `pathParam:"style=simple,explode=false,name=pack"`
	// Unique ID to GET for pack
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// group Id
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *GetPipelinesByPackWithIDRequest) GetPack() string {
	if o == nil {
		return ""
	}
	return o.Pack
}

func (o *GetPipelinesByPackWithIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetPipelinesByPackWithIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// GetPipelinesByPackWithIDResponseBody - a list of Routes objects
type GetPipelinesByPackWithIDResponseBody struct {
	Items []shared.Routes `json:"items,omitempty"`
}

func (o *GetPipelinesByPackWithIDResponseBody) GetItems() []shared.Routes {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetPipelinesByPackWithIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Routes objects
	Object *GetPipelinesByPackWithIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetPipelinesByPackWithIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetPipelinesByPackWithIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetPipelinesByPackWithIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetPipelinesByPackWithIDResponse) GetObject() *GetPipelinesByPackWithIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetPipelinesByPackWithIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
