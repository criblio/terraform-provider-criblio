// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteSchemaByIDServerCloud        string = "cloud"
	DeleteSchemaByIDServerCloudGroup   string = "cloud-group"
	DeleteSchemaByIDServerManagedGroup string = "managed-group"
)

var DeleteSchemaByIDServerList = map[string]string{
	DeleteSchemaByIDServerCloud:        "https://app.cribl.cloud",
	DeleteSchemaByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteSchemaByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteSchemaByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *DeleteSchemaByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *DeleteSchemaByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// DeleteSchemaByIDResponseBody - a list of Schema objects
type DeleteSchemaByIDResponseBody struct {
	Items []shared.SchemaLibEntry `json:"items,omitempty"`
}

func (o *DeleteSchemaByIDResponseBody) GetItems() []shared.SchemaLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteSchemaByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Schema objects
	Object *DeleteSchemaByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteSchemaByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteSchemaByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteSchemaByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteSchemaByIDResponse) GetObject() *DeleteSchemaByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteSchemaByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
