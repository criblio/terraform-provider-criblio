// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetScriptByIDServerCloud        string = "cloud"
	GetScriptByIDServerCloudGroup   string = "cloud-group"
	GetScriptByIDServerManagedGroup string = "managed-group"
)

var GetScriptByIDServerList = map[string]string{
	GetScriptByIDServerCloud:        "https://app.cribl.cloud",
	GetScriptByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetScriptByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetScriptByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetScriptByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetScriptByIDResponseBody - a list of Script objects
type GetScriptByIDResponseBody struct {
	Items []shared.ScriptLibEntry `json:"items,omitempty"`
}

func (o *GetScriptByIDResponseBody) GetItems() []shared.ScriptLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetScriptByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Script objects
	Object *GetScriptByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetScriptByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetScriptByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetScriptByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetScriptByIDResponse) GetObject() *GetScriptByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetScriptByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
