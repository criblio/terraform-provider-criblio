// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	CreateOutputServerCloud        string = "cloud"
	CreateOutputServerCloudGroup   string = "cloud-group"
	CreateOutputServerManagedGroup string = "managed-group"
)

var CreateOutputServerList = map[string]string{
	CreateOutputServerCloud:        "https://app.cribl.cloud",
	CreateOutputServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	CreateOutputServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type CreateOutputRequest struct {
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
	// The consumer group id to create
	ID string `queryParam:"style=form,explode=true,name=id"`
	// New Output object
	Output shared.Output `request:"mediaType=application/json"`
}

func (o *CreateOutputRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

func (o *CreateOutputRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *CreateOutputRequest) GetOutput() shared.Output {
	if o == nil {
		return shared.Output{}
	}
	return o.Output
}

// CreateOutputResponseBody - a list of Output objects
type CreateOutputResponseBody struct {
}

type CreateOutputResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Output objects
	Object *CreateOutputResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *CreateOutputResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *CreateOutputResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *CreateOutputResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *CreateOutputResponse) GetObject() *CreateOutputResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *CreateOutputResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
