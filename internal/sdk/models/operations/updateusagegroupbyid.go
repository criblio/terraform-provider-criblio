// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateUsageGroupByIDServerCloud        string = "cloud"
	UpdateUsageGroupByIDServerCloudGroup   string = "cloud-group"
	UpdateUsageGroupByIDServerManagedGroup string = "managed-group"
)

var UpdateUsageGroupByIDServerList = map[string]string{
	UpdateUsageGroupByIDServerCloud:        "https://app.cribl.cloud",
	UpdateUsageGroupByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateUsageGroupByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateUsageGroupByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// UsageGroup object to be updated
	UsageGroup shared.UsageGroup `request:"mediaType=application/json"`
}

func (o *UpdateUsageGroupByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateUsageGroupByIDRequest) GetUsageGroup() shared.UsageGroup {
	if o == nil {
		return shared.UsageGroup{}
	}
	return o.UsageGroup
}

// UpdateUsageGroupByIDResponseBody - a list of UsageGroup objects
type UpdateUsageGroupByIDResponseBody struct {
	Items []shared.UsageGroup `json:"items,omitempty"`
}

func (o *UpdateUsageGroupByIDResponseBody) GetItems() []shared.UsageGroup {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateUsageGroupByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of UsageGroup objects
	Object *UpdateUsageGroupByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateUsageGroupByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateUsageGroupByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateUsageGroupByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateUsageGroupByIDResponse) GetObject() *UpdateUsageGroupByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateUsageGroupByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
