// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateScriptByIDServerCloud        string = "cloud"
	UpdateScriptByIDServerCloudGroup   string = "cloud-group"
	UpdateScriptByIDServerManagedGroup string = "managed-group"
)

var UpdateScriptByIDServerList = map[string]string{
	UpdateScriptByIDServerCloud:        "https://app.cribl.cloud",
	UpdateScriptByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateScriptByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateScriptByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Script object to be updated
	ScriptLibEntry shared.ScriptLibEntry `request:"mediaType=application/json"`
}

func (o *UpdateScriptByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateScriptByIDRequest) GetScriptLibEntry() shared.ScriptLibEntry {
	if o == nil {
		return shared.ScriptLibEntry{}
	}
	return o.ScriptLibEntry
}

// UpdateScriptByIDResponseBody - a list of Script objects
type UpdateScriptByIDResponseBody struct {
	Items []shared.ScriptLibEntry `json:"items,omitempty"`
}

func (o *UpdateScriptByIDResponseBody) GetItems() []shared.ScriptLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateScriptByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Script objects
	Object *UpdateScriptByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateScriptByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateScriptByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateScriptByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateScriptByIDResponse) GetObject() *UpdateScriptByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateScriptByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
