// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetJobsResultsByIDAndGroupServerCloud        string = "cloud"
	GetJobsResultsByIDAndGroupServerCloudGroup   string = "cloud-group"
	GetJobsResultsByIDAndGroupServerManagedGroup string = "managed-group"
)

var GetJobsResultsByIDAndGroupServerList = map[string]string{
	GetJobsResultsByIDAndGroupServerCloud:        "https://app.cribl.cloud",
	GetJobsResultsByIDAndGroupServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetJobsResultsByIDAndGroupServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetJobsResultsByIDAndGroupRequest struct {
	// Instance id of the job to get results for
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Maximum files to get job results
	MaxFiles *float64 `queryParam:"style=form,explode=true,name=maxFiles"`
	// Group the job belongs to
	Group string `pathParam:"style=simple,explode=false,name=group"`
}

func (o *GetJobsResultsByIDAndGroupRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetJobsResultsByIDAndGroupRequest) GetMaxFiles() *float64 {
	if o == nil {
		return nil
	}
	return o.MaxFiles
}

func (o *GetJobsResultsByIDAndGroupRequest) GetGroup() string {
	if o == nil {
		return ""
	}
	return o.Group
}

// GetJobsResultsByIDAndGroupResponseBody - a list of any objects
type GetJobsResultsByIDAndGroupResponseBody struct {
	Items []map[string]any `json:"items,omitempty"`
}

func (o *GetJobsResultsByIDAndGroupResponseBody) GetItems() []map[string]any {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetJobsResultsByIDAndGroupResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of any objects
	Object *GetJobsResultsByIDAndGroupResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetJobsResultsByIDAndGroupResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetJobsResultsByIDAndGroupResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetJobsResultsByIDAndGroupResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetJobsResultsByIDAndGroupResponse) GetObject() *GetJobsResultsByIDAndGroupResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetJobsResultsByIDAndGroupResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
