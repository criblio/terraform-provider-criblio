// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateProfilerItemByIDServerCloud        string = "cloud"
	UpdateProfilerItemByIDServerCloudGroup   string = "cloud-group"
	UpdateProfilerItemByIDServerManagedGroup string = "managed-group"
)

var UpdateProfilerItemByIDServerList = map[string]string{
	UpdateProfilerItemByIDServerCloud:        "https://app.cribl.cloud",
	UpdateProfilerItemByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateProfilerItemByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateProfilerItemByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// ProfilerItem object to be updated
	ProfilerItem shared.ProfilerItem `request:"mediaType=application/json"`
}

func (o *UpdateProfilerItemByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateProfilerItemByIDRequest) GetProfilerItem() shared.ProfilerItem {
	if o == nil {
		return shared.ProfilerItem{}
	}
	return o.ProfilerItem
}

// UpdateProfilerItemByIDResponseBody - a list of ProfilerItem objects
type UpdateProfilerItemByIDResponseBody struct {
	Items []shared.ProfilerItem `json:"items,omitempty"`
}

func (o *UpdateProfilerItemByIDResponseBody) GetItems() []shared.ProfilerItem {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateProfilerItemByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of ProfilerItem objects
	Object *UpdateProfilerItemByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateProfilerItemByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateProfilerItemByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateProfilerItemByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateProfilerItemByIDResponse) GetObject() *UpdateProfilerItemByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateProfilerItemByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
