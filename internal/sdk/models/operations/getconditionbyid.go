// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetConditionByIDServerCloud        string = "cloud"
	GetConditionByIDServerCloudGroup   string = "cloud-group"
	GetConditionByIDServerManagedGroup string = "managed-group"
)

var GetConditionByIDServerList = map[string]string{
	GetConditionByIDServerCloud:        "https://app.cribl.cloud",
	GetConditionByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetConditionByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetConditionByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetConditionByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetConditionByIDResponseBody - a list of Condition objects
type GetConditionByIDResponseBody struct {
	Items []shared.Condition `json:"items,omitempty"`
}

func (o *GetConditionByIDResponseBody) GetItems() []shared.Condition {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetConditionByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Condition objects
	Object *GetConditionByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetConditionByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetConditionByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetConditionByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetConditionByIDResponse) GetObject() *GetConditionByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetConditionByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
