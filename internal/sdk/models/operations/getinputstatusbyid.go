// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetInputStatusByIDServerCloud        string = "cloud"
	GetInputStatusByIDServerCloudGroup   string = "cloud-group"
	GetInputStatusByIDServerManagedGroup string = "managed-group"
)

var GetInputStatusByIDServerList = map[string]string{
	GetInputStatusByIDServerCloud:        "https://app.cribl.cloud",
	GetInputStatusByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetInputStatusByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetInputStatusByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetInputStatusByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetInputStatusByIDResponseBody - a list of InputStatus objects
type GetInputStatusByIDResponseBody struct {
	Items []shared.InputStatus `json:"items,omitempty"`
}

func (o *GetInputStatusByIDResponseBody) GetItems() []shared.InputStatus {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetInputStatusByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of InputStatus objects
	Object *GetInputStatusByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetInputStatusByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetInputStatusByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetInputStatusByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetInputStatusByIDResponse) GetObject() *GetInputStatusByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetInputStatusByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
