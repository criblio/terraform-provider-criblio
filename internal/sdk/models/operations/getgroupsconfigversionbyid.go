// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetGroupsConfigVersionByIDServerCloud        string = "cloud"
	GetGroupsConfigVersionByIDServerCloudGroup   string = "cloud-group"
	GetGroupsConfigVersionByIDServerManagedGroup string = "managed-group"
)

var GetGroupsConfigVersionByIDServerList = map[string]string{
	GetGroupsConfigVersionByIDServerCloud:        "https://app.cribl.cloud",
	GetGroupsConfigVersionByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetGroupsConfigVersionByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetGroupsConfigVersionByIDRequest struct {
	// Group ID
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetGroupsConfigVersionByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetGroupsConfigVersionByIDResponseBody - a list of string objects
type GetGroupsConfigVersionByIDResponseBody struct {
	Items []string `json:"items,omitempty"`
}

func (o *GetGroupsConfigVersionByIDResponseBody) GetItems() []string {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetGroupsConfigVersionByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of string objects
	Object *GetGroupsConfigVersionByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetGroupsConfigVersionByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetGroupsConfigVersionByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetGroupsConfigVersionByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetGroupsConfigVersionByIDResponse) GetObject() *GetGroupsConfigVersionByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetGroupsConfigVersionByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
