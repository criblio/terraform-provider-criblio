// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetChangelogViewedServerCloud        string = "cloud"
	GetChangelogViewedServerCloudGroup   string = "cloud-group"
	GetChangelogViewedServerManagedGroup string = "managed-group"
)

var GetChangelogViewedServerList = map[string]string{
	GetChangelogViewedServerCloud:        "https://app.cribl.cloud",
	GetChangelogViewedServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetChangelogViewedServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

// GetChangelogViewedResponseBody - a list of ChangelogState objects
type GetChangelogViewedResponseBody struct {
	Items []shared.ChangelogState `json:"items,omitempty"`
}

func (o *GetChangelogViewedResponseBody) GetItems() []shared.ChangelogState {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetChangelogViewedResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of ChangelogState objects
	Object *GetChangelogViewedResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetChangelogViewedResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetChangelogViewedResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetChangelogViewedResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetChangelogViewedResponse) GetObject() *GetChangelogViewedResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetChangelogViewedResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
