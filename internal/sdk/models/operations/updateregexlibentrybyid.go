// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateRegexLibEntryByIDServerCloud        string = "cloud"
	UpdateRegexLibEntryByIDServerCloudGroup   string = "cloud-group"
	UpdateRegexLibEntryByIDServerManagedGroup string = "managed-group"
)

var UpdateRegexLibEntryByIDServerList = map[string]string{
	UpdateRegexLibEntryByIDServerCloud:        "https://app.cribl.cloud",
	UpdateRegexLibEntryByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateRegexLibEntryByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateRegexLibEntryByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
	// RegexLibEntry object to be updated
	RegexLibEntry shared.RegexLibEntry `request:"mediaType=application/json"`
}

func (o *UpdateRegexLibEntryByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateRegexLibEntryByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

func (o *UpdateRegexLibEntryByIDRequest) GetRegexLibEntry() shared.RegexLibEntry {
	if o == nil {
		return shared.RegexLibEntry{}
	}
	return o.RegexLibEntry
}

// UpdateRegexLibEntryByIDResponseBody - a list of RegexLibEntry objects
type UpdateRegexLibEntryByIDResponseBody struct {
	Items []shared.RegexLibEntry `json:"items,omitempty"`
}

func (o *UpdateRegexLibEntryByIDResponseBody) GetItems() []shared.RegexLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateRegexLibEntryByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of RegexLibEntry objects
	Object *UpdateRegexLibEntryByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateRegexLibEntryByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateRegexLibEntryByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateRegexLibEntryByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateRegexLibEntryByIDResponse) GetObject() *UpdateRegexLibEntryByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateRegexLibEntryByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
