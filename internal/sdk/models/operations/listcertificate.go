// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	ListCertificateServerCloud        string = "cloud"
	ListCertificateServerCloudGroup   string = "cloud-group"
	ListCertificateServerManagedGroup string = "managed-group"
)

var ListCertificateServerList = map[string]string{
	ListCertificateServerCloud:        "https://app.cribl.cloud",
	ListCertificateServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	ListCertificateServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

// ListCertificateResponseBody - a list of Certificate objects
type ListCertificateResponseBody struct {
	Items []shared.Certificate `json:"items,omitempty"`
}

func (o *ListCertificateResponseBody) GetItems() []shared.Certificate {
	if o == nil {
		return nil
	}
	return o.Items
}

type ListCertificateResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Certificate objects
	Object *ListCertificateResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *ListCertificateResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *ListCertificateResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *ListCertificateResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *ListCertificateResponse) GetObject() *ListCertificateResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *ListCertificateResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
