// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetDatasetACLTeamsByIDServerCloud        string = "cloud"
	GetDatasetACLTeamsByIDServerCloudGroup   string = "cloud-group"
	GetDatasetACLTeamsByIDServerManagedGroup string = "managed-group"
)

var GetDatasetACLTeamsByIDServerList = map[string]string{
	GetDatasetACLTeamsByIDServerCloud:        "https://app.cribl.cloud",
	GetDatasetACLTeamsByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetDatasetACLTeamsByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetDatasetACLTeamsByIDRequest struct {
	// Unique ID for ACL Teams Get
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetDatasetACLTeamsByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetDatasetACLTeamsByIDResponseBody - a list of UserAccessControlList objects
type GetDatasetACLTeamsByIDResponseBody struct {
	Items []shared.UserAccessControlList `json:"items,omitempty"`
}

func (o *GetDatasetACLTeamsByIDResponseBody) GetItems() []shared.UserAccessControlList {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetDatasetACLTeamsByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of UserAccessControlList objects
	Object *GetDatasetACLTeamsByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetDatasetACLTeamsByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetDatasetACLTeamsByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetDatasetACLTeamsByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetDatasetACLTeamsByIDResponse) GetObject() *GetDatasetACLTeamsByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetDatasetACLTeamsByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
