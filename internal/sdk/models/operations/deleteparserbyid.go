// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteParserByIDServerCloud        string = "cloud"
	DeleteParserByIDServerCloudGroup   string = "cloud-group"
	DeleteParserByIDServerManagedGroup string = "managed-group"
)

var DeleteParserByIDServerList = map[string]string{
	DeleteParserByIDServerCloud:        "https://app.cribl.cloud",
	DeleteParserByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteParserByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteParserByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *DeleteParserByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *DeleteParserByIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// DeleteParserByIDResponseBody - a list of Parser objects
type DeleteParserByIDResponseBody struct {
	Items []shared.ParserLibEntry `json:"items,omitempty"`
}

func (o *DeleteParserByIDResponseBody) GetItems() []shared.ParserLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteParserByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Parser objects
	Object *DeleteParserByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteParserByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteParserByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteParserByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteParserByIDResponse) GetObject() *DeleteParserByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteParserByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
