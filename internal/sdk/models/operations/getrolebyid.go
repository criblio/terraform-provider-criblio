// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetRoleByIDServerCloud        string = "cloud"
	GetRoleByIDServerCloudGroup   string = "cloud-group"
	GetRoleByIDServerManagedGroup string = "managed-group"
)

var GetRoleByIDServerList = map[string]string{
	GetRoleByIDServerCloud:        "https://app.cribl.cloud",
	GetRoleByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetRoleByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetRoleByIDRequest struct {
	// Unique ID to GET
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetRoleByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// GetRoleByIDResponseBody - a list of Role objects
type GetRoleByIDResponseBody struct {
	Items []shared.Role `json:"items,omitempty"`
}

func (o *GetRoleByIDResponseBody) GetItems() []shared.Role {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetRoleByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Role objects
	Object *GetRoleByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetRoleByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetRoleByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetRoleByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetRoleByIDResponse) GetObject() *GetRoleByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetRoleByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
