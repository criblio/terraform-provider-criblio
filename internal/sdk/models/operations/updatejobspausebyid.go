// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateJobsPauseByIDServerCloud        string = "cloud"
	UpdateJobsPauseByIDServerCloudGroup   string = "cloud-group"
	UpdateJobsPauseByIDServerManagedGroup string = "managed-group"
)

var UpdateJobsPauseByIDServerList = map[string]string{
	UpdateJobsPauseByIDServerCloud:        "https://app.cribl.cloud",
	UpdateJobsPauseByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateJobsPauseByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateJobsPauseByIDRequest struct {
	// Job instance id
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *UpdateJobsPauseByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// UpdateJobsPauseByIDResponseBody - a list of JobState objects
type UpdateJobsPauseByIDResponseBody struct {
	Items []shared.JobState `json:"items,omitempty"`
}

func (o *UpdateJobsPauseByIDResponseBody) GetItems() []shared.JobState {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateJobsPauseByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of JobState objects
	Object *UpdateJobsPauseByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateJobsPauseByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateJobsPauseByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateJobsPauseByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateJobsPauseByIDResponse) GetObject() *UpdateJobsPauseByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateJobsPauseByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
