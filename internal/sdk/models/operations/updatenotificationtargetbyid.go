// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateNotificationTargetByIDServerCloud        string = "cloud"
	UpdateNotificationTargetByIDServerCloudGroup   string = "cloud-group"
	UpdateNotificationTargetByIDServerManagedGroup string = "managed-group"
)

var UpdateNotificationTargetByIDServerList = map[string]string{
	UpdateNotificationTargetByIDServerCloud:        "https://app.cribl.cloud",
	UpdateNotificationTargetByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateNotificationTargetByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateNotificationTargetByIDRequest struct {
	// Unique ID to PATCH
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// NotificationTarget object to be updated
	NotificationTarget shared.NotificationTarget `request:"mediaType=application/json"`
}

func (o *UpdateNotificationTargetByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateNotificationTargetByIDRequest) GetNotificationTarget() shared.NotificationTarget {
	if o == nil {
		return shared.NotificationTarget{}
	}
	return o.NotificationTarget
}

// UpdateNotificationTargetByIDResponseBody - a list of NotificationTarget objects
type UpdateNotificationTargetByIDResponseBody struct {
}

type UpdateNotificationTargetByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of NotificationTarget objects
	Object *UpdateNotificationTargetByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateNotificationTargetByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateNotificationTargetByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateNotificationTargetByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateNotificationTargetByIDResponse) GetObject() *UpdateNotificationTargetByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateNotificationTargetByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
