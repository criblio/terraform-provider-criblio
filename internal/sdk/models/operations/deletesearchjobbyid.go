// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteSearchJobByIDServerCloud        string = "cloud"
	DeleteSearchJobByIDServerCloudGroup   string = "cloud-group"
	DeleteSearchJobByIDServerManagedGroup string = "managed-group"
)

var DeleteSearchJobByIDServerList = map[string]string{
	DeleteSearchJobByIDServerCloud:        "https://app.cribl.cloud",
	DeleteSearchJobByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteSearchJobByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteSearchJobByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteSearchJobByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteSearchJobByIDResponseBody - a list of SearchJob objects
type DeleteSearchJobByIDResponseBody struct {
	Items []shared.SearchJob `json:"items,omitempty"`
}

func (o *DeleteSearchJobByIDResponseBody) GetItems() []shared.SearchJob {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteSearchJobByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of SearchJob objects
	Object *DeleteSearchJobByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteSearchJobByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteSearchJobByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteSearchJobByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteSearchJobByIDResponse) GetObject() *DeleteSearchJobByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteSearchJobByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
