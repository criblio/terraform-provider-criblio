// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteScriptByIDServerCloud        string = "cloud"
	DeleteScriptByIDServerCloudGroup   string = "cloud-group"
	DeleteScriptByIDServerManagedGroup string = "managed-group"
)

var DeleteScriptByIDServerList = map[string]string{
	DeleteScriptByIDServerCloud:        "https://app.cribl.cloud",
	DeleteScriptByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteScriptByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteScriptByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteScriptByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteScriptByIDResponseBody - a list of Script objects
type DeleteScriptByIDResponseBody struct {
	Items []shared.ScriptLibEntry `json:"items,omitempty"`
}

func (o *DeleteScriptByIDResponseBody) GetItems() []shared.ScriptLibEntry {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteScriptByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Script objects
	Object *DeleteScriptByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteScriptByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteScriptByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteScriptByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteScriptByIDResponse) GetObject() *DeleteScriptByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteScriptByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
