// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetGroupsByIDServerCloud        string = "cloud"
	GetGroupsByIDServerCloudGroup   string = "cloud-group"
	GetGroupsByIDServerManagedGroup string = "managed-group"
)

var GetGroupsByIDServerList = map[string]string{
	GetGroupsByIDServerCloud:        "https://app.cribl.cloud",
	GetGroupsByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetGroupsByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetGroupsByIDRequest struct {
	// Group id
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// fields to add to results: git.commit, git.localChanges, git.log
	Fields *string `queryParam:"style=form,explode=true,name=fields"`
}

func (o *GetGroupsByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetGroupsByIDRequest) GetFields() *string {
	if o == nil {
		return nil
	}
	return o.Fields
}

// GetGroupsByIDResponseBody - a list of ConfigGroup objects
type GetGroupsByIDResponseBody struct {
	Items []shared.Group `json:"items,omitempty"`
}

func (o *GetGroupsByIDResponseBody) GetItems() []shared.Group {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetGroupsByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of ConfigGroup objects
	Object *GetGroupsByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetGroupsByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetGroupsByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetGroupsByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetGroupsByIDResponse) GetObject() *GetGroupsByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetGroupsByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
