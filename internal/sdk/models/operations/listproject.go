// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	ListProjectServerCloud        string = "cloud"
	ListProjectServerCloudGroup   string = "cloud-group"
	ListProjectServerManagedGroup string = "managed-group"
)

var ListProjectServerList = map[string]string{
	ListProjectServerCloud:        "https://app.cribl.cloud",
	ListProjectServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	ListProjectServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type ListProjectRequest struct {
	// The consumer group to which this instance belongs. Defaults to 'Cribl'.
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
}

func (o *ListProjectRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

// ListProjectResponseBody - a list of Project objects
type ListProjectResponseBody struct {
	Items []shared.ProjectConfig `json:"items,omitempty"`
}

func (o *ListProjectResponseBody) GetItems() []shared.ProjectConfig {
	if o == nil {
		return nil
	}
	return o.Items
}

type ListProjectResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of Project objects
	Object *ListProjectResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *ListProjectResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *ListProjectResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *ListProjectResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *ListProjectResponse) GetObject() *ListProjectResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *ListProjectResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
