// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerCloud        string = "cloud"
	GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerCloudGroup   string = "cloud-group"
	GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerManagedGroup string = "managed-group"
)

var GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerList = map[string]string{
	GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerCloud:        "https://app.cribl.cloud",
	GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDRequest struct {
	// Group Id
	GroupID string `pathParam:"style=simple,explode=false,name=groupId"`
	// Project Id
	ProjectID string `pathParam:"style=simple,explode=false,name=projectId"`
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDRequest) GetGroupID() string {
	if o == nil {
		return ""
	}
	return o.GroupID
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDRequest) GetProjectID() string {
	if o == nil {
		return ""
	}
	return o.ProjectID
}

// GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponseBody - A list of Subscription objects
type GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponseBody struct {
	Items []shared.Subscription `json:"items,omitempty"`
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponseBody) GetItems() []shared.Subscription {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// A list of Subscription objects
	Object *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponse) GetObject() *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetSystemProjectsSubscriptionsByGroupIDByAndProjectIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
