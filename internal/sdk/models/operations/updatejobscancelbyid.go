// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	UpdateJobsCancelByIDServerCloud        string = "cloud"
	UpdateJobsCancelByIDServerCloudGroup   string = "cloud-group"
	UpdateJobsCancelByIDServerManagedGroup string = "managed-group"
)

var UpdateJobsCancelByIDServerList = map[string]string{
	UpdateJobsCancelByIDServerCloud:        "https://app.cribl.cloud",
	UpdateJobsCancelByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	UpdateJobsCancelByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type UpdateJobsCancelByIDRequest struct {
	// Job instance id
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *UpdateJobsCancelByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// UpdateJobsCancelByIDResponseBody - a list of JobState objects
type UpdateJobsCancelByIDResponseBody struct {
	Items []shared.JobState `json:"items,omitempty"`
}

func (o *UpdateJobsCancelByIDResponseBody) GetItems() []shared.JobState {
	if o == nil {
		return nil
	}
	return o.Items
}

type UpdateJobsCancelByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of JobState objects
	Object *UpdateJobsCancelByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *UpdateJobsCancelByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateJobsCancelByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateJobsCancelByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *UpdateJobsCancelByIDResponse) GetObject() *UpdateJobsCancelByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *UpdateJobsCancelByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
