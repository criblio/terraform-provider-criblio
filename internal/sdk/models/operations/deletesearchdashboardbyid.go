// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	DeleteSearchDashboardByIDServerCloud        string = "cloud"
	DeleteSearchDashboardByIDServerCloudGroup   string = "cloud-group"
	DeleteSearchDashboardByIDServerManagedGroup string = "managed-group"
)

var DeleteSearchDashboardByIDServerList = map[string]string{
	DeleteSearchDashboardByIDServerCloud:        "https://app.cribl.cloud",
	DeleteSearchDashboardByIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	DeleteSearchDashboardByIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type DeleteSearchDashboardByIDRequest struct {
	// Unique ID to DELETE
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *DeleteSearchDashboardByIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

// DeleteSearchDashboardByIDResponseBody - a list of SearchDashboard objects
type DeleteSearchDashboardByIDResponseBody struct {
	Items []shared.SearchDashboard `json:"items,omitempty"`
}

func (o *DeleteSearchDashboardByIDResponseBody) GetItems() []shared.SearchDashboard {
	if o == nil {
		return nil
	}
	return o.Items
}

type DeleteSearchDashboardByIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of SearchDashboard objects
	Object *DeleteSearchDashboardByIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *DeleteSearchDashboardByIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteSearchDashboardByIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteSearchDashboardByIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteSearchDashboardByIDResponse) GetObject() *DeleteSearchDashboardByIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *DeleteSearchDashboardByIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
