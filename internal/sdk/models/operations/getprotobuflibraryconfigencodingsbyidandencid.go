// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/terraform-provider-criblio/internal/sdk/models/shared"
	"net/http"
)

const (
	GetProtobufLibraryConfigEncodingsByIDAndEncIDServerCloud        string = "cloud"
	GetProtobufLibraryConfigEncodingsByIDAndEncIDServerCloudGroup   string = "cloud-group"
	GetProtobufLibraryConfigEncodingsByIDAndEncIDServerManagedGroup string = "managed-group"
)

var GetProtobufLibraryConfigEncodingsByIDAndEncIDServerList = map[string]string{
	GetProtobufLibraryConfigEncodingsByIDAndEncIDServerCloud:        "https://app.cribl.cloud",
	GetProtobufLibraryConfigEncodingsByIDAndEncIDServerCloudGroup:   "https://{workspaceName}-{organizationId}.{cloudDomain}/api/v1/m/{groupName}",
	GetProtobufLibraryConfigEncodingsByIDAndEncIDServerManagedGroup: "https://{hostname}:{port}/api/v1/m/{groupName}",
}

type GetProtobufLibraryConfigEncodingsByIDAndEncIDRequest struct {
	// Library id
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Encoding id
	Encid string `pathParam:"style=simple,explode=false,name=encid"`
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDRequest) GetEncid() string {
	if o == nil {
		return ""
	}
	return o.Encid
}

// GetProtobufLibraryConfigEncodingsByIDAndEncIDResponseBody - a list of ProtobufEncodingConfig objects
type GetProtobufLibraryConfigEncodingsByIDAndEncIDResponseBody struct {
	Items []shared.ProtobufEncodingConfig `json:"items,omitempty"`
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponseBody) GetItems() []shared.ProtobufEncodingConfig {
	if o == nil {
		return nil
	}
	return o.Items
}

type GetProtobufLibraryConfigEncodingsByIDAndEncIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// a list of ProtobufEncodingConfig objects
	Object *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponseBody
	// Unexpected error
	Error *shared.Error
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponse) GetObject() *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponseBody {
	if o == nil {
		return nil
	}
	return o.Object
}

func (o *GetProtobufLibraryConfigEncodingsByIDAndEncIDResponse) GetError() *shared.Error {
	if o == nil {
		return nil
	}
	return o.Error
}
