// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package types

import (
	"github.com/hashicorp/terraform-plugin-framework/types"
)

type InputOpenai1 struct {
	TemplateOpenaiOrganization types.String                   `tfsdk:"template_openai_organization"`
	TemplateOpenaiProject      types.String                   `tfsdk:"template_openai_project"`
	APIKey                     types.String                   `tfsdk:"api_key"`
	Connections                []ItemsTypeConnectionsOptional `tfsdk:"connections"`
	ContentConfig              []InputOpenaiContentConfig     `tfsdk:"content_config"`
	Description                types.String                   `tfsdk:"description"`
	Disabled                   types.Bool                     `tfsdk:"disabled"`
	Environment                types.String                   `tfsdk:"environment"`
	ID                         types.String                   `tfsdk:"id"`
	IgnoreGroupJobsLimit       types.Bool                     `tfsdk:"ignore_group_jobs_limit"`
	KeepAliveTime              types.Float64                  `tfsdk:"keep_alive_time"`
	MaxMissedKeepAlives        types.Float64                  `tfsdk:"max_missed_keep_alives"`
	Metadata                   []ItemsTypeMetadata            `tfsdk:"metadata"`
	OpenaiOrganization         types.String                   `tfsdk:"openai_organization"`
	OpenaiProject              types.String                   `tfsdk:"openai_project"`
	Pipeline                   types.String                   `tfsdk:"pipeline"`
	Pq                         *PqType                        `tfsdk:"pq"`
	PqEnabled                  types.Bool                     `tfsdk:"pq_enabled"`
	RequestTimeout             types.Float64                  `tfsdk:"request_timeout"`
	RetryRules                 *RetryRulesType                `tfsdk:"retry_rules"`
	SendToRoutes               types.Bool                     `tfsdk:"send_to_routes"`
	Streamtags                 []types.String                 `tfsdk:"streamtags"`
	TextSecret                 types.String                   `tfsdk:"text_secret"`
	TTL                        types.String                   `tfsdk:"ttl"`
	Type                       types.String                   `tfsdk:"type"`
}
