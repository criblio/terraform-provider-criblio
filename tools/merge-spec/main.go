...
func runWithConfig(config mergeConfig) error {
	...
	cloudOnlyPaths, onPremOnlyPaths, err := collectAvailabilityPaths(spec)
	if err != nil {
		return err
	}
	...
	if err := writeAvailabilityPaths(config.CloudOnlyPathsOutputPath, cloudOnlyPaths, onPremOnlyPaths); err != nil {
		return err
	}
	return nil
}
...
func collectAvailabilityPaths(spec *yaml.Node) ([]string, []string, error) {
	...
	if !ok || paths.Kind != yaml.MappingNode {
		return nil, nil, fmt.Errorf("spec paths mapping not found")
	}

	var cloudOnlyPaths []string
	var onPremOnlyPaths []string
	for index := 0; index < len(paths.Content); index += 2 {
		...
		normalized := strings.TrimPrefix(path, groupPrefix)
		switch pathAvailability(pathItem) {
		case "cloud":
			cloudOnlyPaths = append(cloudOnlyPaths, normalized)
		case "onprem":
			onPremOnlyPaths = append(onPremOnlyPaths, normalized)
		}
	}
...
func pathAvailability(pathItem *yaml.Node) string {
	...
	for index := 0; index < len(pathItem.Content); index += 2 {
		...
		if !ok || availability.Kind != yaml.ScalarNode {
			return ""
		}
		if availability.Value != "cloud" && availability.Value != "onprem" {
			return ""
		}
		if availabilityValue != "" && availabilityValue != availability.Value {
			return ""
		}
...
func writeAvailabilityPaths(filename string, cloudOnlyPaths, onPremOnlyPaths []string) error {
	...
	for _, path := range cloudOnlyPaths {
		fmt.Fprintf(&output, "\t\t%q: true,\n", path)
	}
	output.WriteString("\t}\n")
	output.WriteString("\tonPremOnlyPaths = map[string]bool{\n")
	for _, path := range onPremOnlyPaths {
		fmt.Fprintf(&output, "\t\t%q: true,\n", path)
	}
	output.WriteString("\t}\n")
	output.WriteString("}\n")
