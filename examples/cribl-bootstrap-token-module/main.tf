...
locals {
  ...
  bootstrap_script_command = var.create_script_file ? "./${var.script_filename} -i '${var.client_id}' -s '${var.client_secret}' -o '${var.organization}' -w '${var.workspace}' -e '${var.environment}' -g '${var.group}' > ${var.token_output_file}" : "bash -c '${local.bootstrap_script}' > ${var.token_output_file}"
} 